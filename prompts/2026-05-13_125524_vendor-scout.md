...
timestamp: 2026-05-13T12:55:24.901053
---
...
## Title
AMD Introduces Air‑Cooled MI350P GPU for On‑Premise HPC/AI Workloads  

**Source URL**: https://www.nextplatform.com/compute/2026/05/08/sometimes-air-is-the-only-way-for-ai-systems-to-keep-their-cool/5237421  
**Published Date**: 2026‑05‑08T18:09:19+02:00  
**Source Credibility**: **High** – The Next Platform is a reputable industry‑focused publication with editorial oversight and frequent citations from vendor roadmaps.  

### Summary
AMD has released a new variant of its Instinct MI350 series, the **MI350P**, designed for **air‑cooled** server environments. The card is a **half‑capacity** version of the MI350X, occupying a standard PCI‑Express form factor that fits conventional rack servers. It retains the CDNA 4 architecture, 12 HBM3E memory stacks, and supports the OCP‑FP8, MXFP6, and MXFP4 data formats. Reported performance figures show the MI350P delivering **≈90 % of the 4 TB/s peak memory bandwidth** and **50‑66 % of peak FLOPs** depending on precision, based on AMD’s internal benchmarks (specific test suite not disclosed).

Thermal design power (TDP) is **600 W** at the nominal 2.2 GHz clock, but AMD provides a **450 W power‑down mode** (≈10‑15 % clock reduction) that is expected to cut performance by only 10‑15 % for bandwidth‑bound workloads. The card targets **200‑250 B‑parameter models** for inference and modest training, fitting use cases where liquid cooling is impractical (e.g., financial‑services desks, manufacturing sites, and other on‑premise environments). OEMs such as Dell, HPE, Lenovo, Cisco, and Supermicro have announced upcoming server platforms (e.g., Dell PowerEdge XE7745, HPE ProLiant DL385/345, Lenovo ThinkSystem SR675, Cisco C845a) that will integrate the MI350P, typically paired with one or two Genoa (EPYC 9004) or Turin (EPYC 9005) CPUs.

Pricing has not been disclosed, but AMD suggests the MI350P will cost **≈50 % of the MI350X** price, reflecting its reduced memory‑coherency capabilities (no two‑way GPU‑GPU or GPU‑CPU coherency). Availability is expected to be limited, similar to other high‑end GPUs.

### Operational Impact
- **Server Planning**: HPC admins can now consider high‑performance GPU acceleration in **air‑cooled racks** without needing dedicated liquid‑cooling infrastructure, expanding upgrade paths for existing data centers in space‑constrained or regulated locations.
- **Power & Cooling Budgets**: The 450 W throttled mode offers a **lower‑power tier** that still retains >90 % of memory bandwidth, enabling tighter PUE targets and simplifying rack power distribution design.
- **Node Design**: Expect new OEM reference designs that combine **4‑8 MI350P GPUs** with **2× EPYC Genoa/Turin CPUs**. Administrators should verify BIOS/firmware support for PCIe 5.0 x16 lanes and ensure adequate airflow (front‑to‑back) to sustain the 600 W mode if full performance is required.
- **Software Stack**: Since the MI350P lacks GPU‑GPU memory coherency, multi‑GPU scaling will rely on explicit data movement (e.g., via RCCL/ROCm). Existing container orchestration (Kubernetes, Slurm) will need to be configured for **stand‑alone GPU isolation** rather than shared coherency domains.
- **Workload Suitability**: The card is optimized for **inference on 200‑250 B‑parameter models** and modest training; workloads demanding >300 B parameters or heavy inter‑GPU communication may see degraded efficiency compared to MI350X/MI355X deployments.

### Key Technical Details
- **GPU Architecture**: CDNA 4, 12 × HBM3E stacks, OCP‑FP8 / MXFP6 / MXFP4 support.  
- **Form Factor**: Standard PCI‑Express (air‑cooled).  
- **TDP**: 600 W (full) / 450 W (throttled).  
- **Clock**: Nominal 2.2 GHz; throttled mode ≈1.9‑2.0 GHz.  
- **Performance**: 90 % of 4 TB/s memory bandwidth; 50‑66 % of peak FLOPs (precision‑dependent).  
- **Target Model Size**: 200‑250 B parameters (inference).  
- **OEM Server Platforms**: Dell XE7745 / R7725, HPE ProLiant DL385‑Gen11, DL345‑Gen12, Lenovo SR675/I v3, Cisco C845a M8 / X Series 580p / UC245 M8, Supermicro AS‑5126GS‑TNRT series, etc.  
- **Memory Coherency**: No two‑way GPU‑GPU or GPU‑CPU coherency; standalone operation only.  

### Tags
[GPU] [air‑cooling] [power management] [OEM] [CDNA 4] [HBM3E] [inference] [training]  

### Importance
**High** – Introduces a new class of high‑performance, air‑cooled GPUs that directly affect hardware refresh cycles, power budgeting, and workload placement decisions for on‑premise HPC/AI clusters.

### Why This Matters
The MI350P enables organizations that cannot deploy liquid‑cooling to adopt cutting‑edge GPU acceleration, forcing HPC administrators to reassess node designs, power/cooling allocations, and software orchestration for standalone, high‑throughput AI inference workloads.
