...
timestamp: 2026-05-13T12:56:46.402472
---
...
**Source URL**: https://www.nextplatform.com/connect/2026/05/12/openai-microsoft-and-friends-build-a-better-more-scalable-ethernet/5239078  
**Published Date**: 2026-05-12T19:52:33+02:00  
**Source Credibility**: **High** – The Next Platform is a well‑established, peer‑reviewed industry news outlet with a strong track record for accurate technical reporting.  

---

### Summary
Researchers from OpenAI, Microsoft, Broadcom, AMD, and Nvidia have introduced **Multipath Reliable Connection (MRC)**, a superset extension of RoCE‑v2 that runs atop existing Ethernet switch ASICs. MRC adopts concepts from the Ultra Ethernet specification (Ultra Ethernet Consortium, July 2023) to provide InfiniBand‑class latency while retaining Ethernet’s ubiquity. The protocol leverages **high‑radix switches** (e.g., 512 × 100 Gb/s ports per ASIC) and creates **eight parallel Clos data planes** per endpoint, dramatically increasing the number of physical links between compute nodes without raising per‑port bandwidth.

Key technical mechanisms include:
- **Explicit Congestion Notification (ECN)‑based adaptive load balancing** with packet spraying across the eight links.  
- **Packet trimming**: selective retransmission of only the payload of dropped packets, avoiding full‑packet ECN throttling; requires ASIC‑level support (implemented in Nvidia’s Cumulus‑Linux‑derived OS and Mellanox‑derived firmware).  
- **IPv6 Segment Routing (SRv6) static routing**: disables dynamic routing in favor of pre‑computed paths, simplifying forwarding across the multiple planes.  
- **Fault tolerance**: loss of a single link reduces bandwidth by ~12 % per endpoint; the network can heal around failed links or switches without halting a synchronous AI training job.

The authors compare a traditional three‑tier RoCE network (51.2 Tb/s, 800 Gb/s ports, 5,120 switches for 65 k GPUs) with an MRC‑enabled two‑tier network (same aggregate bandwidth, 512 × 100 Gb/s ports, 6,144 switches for 131 k GPUs). While the MRC design uses ~20 % more switches, it halves the hop count (max three hops) and provides **full‑bisection bandwidth with 2/3 the optics and 3/5 the switches** for a given scale. Real‑world deployments are reported at Oracle’s Stargate and Microsoft Azure AI datacenters, using Nvidia ConnectX‑8 SmartNICs, AMD “Pollara”/“Vulcano” DPUs, Broadcom Thor Ultra SmartNICs, and switches from Nvidia Spectrum 4/5, Arista EOS, and Cumulus/SONiC OSes.

---

### Operational Impact
- **Network redesign**: HPC sites planning >65 k GPU/XPU nodes should evaluate a shift from traditional three‑tier RoCE Clos fabrics to a high‑radix, two‑tier MRC topology to reduce latency and improve fault tolerance.  
- **Hardware procurement**: Adoption requires switches capable of 512 × 100 Gb/s ports (e.g., Broadcom Tomahawk 5, Nvidia Spectrum 5) and NICs/DPUs that support the MRC extensions (ConnectX‑8, AMD Pollara/Vulcano, Broadcom Thor). Existing RoCE‑v2 hardware may need firmware upgrades; verify vendor road‑maps.  
- **Software stack changes**: Enable SRv6 static routing and packet‑trimming features in the network OS (Cumulus, SONiC, EOS). Update job schedulers (Slurm, PBS) to be aware of the multi‑path topology if they perform network‑aware placement or QoS.  
- **Fault‑tolerance procedures**: Redefine checkpoint/recovery policies. Since a single link loss no longer forces a full job restart, administrators can relax aggressive checkpoint intervals, but must still monitor link health and implement out‑of‑band predictive failure detection as suggested.  
- **Capacity planning**: While switch count rises modestly, the **link budget** grows substantially (up to ~1 M copper/optical cables for a 65 k‑node cluster). Cabling costs and physical layout become primary constraints; plan for dense DAC bundles within racks and high‑density optical transceivers for inter‑rack links.  
- **Power & TCO**: Fewer spine/superspine switches lower overall power draw, potentially offsetting the extra switch count. Administrators should recalculate PUE and cooling requirements under the new topology.

---

### Key Technical Details
- **Protocol**: Multipath Reliable Connection (MRC), superset of RoCE‑v2, incorporates ECN‑based load balancing, packet trimming, out‑of‑order delivery, and SRv6 static routing.  
- **Switch ASIC**: 512 ports @ 100 Gb/s (derived from splitting a 51.2 Tb/s ASIC).  
- **Topology**: Two‑tier Clos with eight parallel data planes; each endpoint connects via eight 100 Gb/s links.  
- **Scale**: 131 072 GPUs/XPUs on a 6 144‑switch fabric; max three hops between any two nodes.  
- **Fault tolerance**: Single‑link loss → ~12 % bandwidth reduction; network self‑heals without job interruption.  
- **Deployments**: Oracle Stargate (TX) and Microsoft Azure AI (WI).  
- **Hardware support**: Nvidia ConnectX‑8 SmartNICs, AMD “Pollara”/“Vulcano” DPUs, Broadcom Thor Ultra SmartNICs; switches: Nvidia Spectrum 4/5 (Cumulus/SONiC), Arista EOS (Broadcom Tomahawk 5).  

---

### Tags
[networking] [GPU] [orchestration] [vendor] [research] [observability]  

---

### Importance
**High** – The protocol directly affects network architecture, fault tolerance, and performance of large‑scale AI training clusters, which are core workloads for many HPC centers.

---

### Why This Matters
MRC offers a path to double the compute density of AI clusters while cutting latency and improving resilience, forcing HPC administrators to reconsider traditional three‑tier RoCE fabrics and to plan for new hardware, firmware, and operational practices.
