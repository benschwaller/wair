...
timestamp: 2026-05-13T12:54:00.663820
---
...
## Multipath Reliable Connection (MRC): A New Ethernet Protocol for Large-Scale AI Clusters

**Source**: The Next Platform | **Date**: 2026-05-12 | **Credibility**: [High Credibility]

**Summary**:
OpenAI, Microsoft, Broadcom, AMD, and Nvidia have developed Multipath Reliable Connection (MRC), a new network protocol that fundamentally changes how Ethernet is used for AI infrastructure. Rather than chasing higher port bandwidth (800 Gb/sec), MRC uses the aggregate bandwidth of switch ASICs to increase the number of links between devices—splitting a 51.2 Tb/sec switch into 512 ports at 100 Gb/sec each. The protocol creates eight parallel Clos data planes (instead of one), enabling a two-tier network topology that connects 131,072 compute engines with only three hops between any endpoints (versus five to seven hops in traditional three-tier architectures). MRC is implemented as a superset extension of RDMA over Converged Ethernet (RoCE) and uses IPv6 segment routing (SRv6) for static routing, with adaptive load balancing based on Explicit Congestion Notification (ECN). The protocol supports out-of-order delivery, packet spraying across multiple links, selective retransmission, and packet trimming to handle congestion without invoking global ECN mechanisms.

**Operational Impact**:
For HPC administrators, MRC represents a significant shift in network architecture planning for AI clusters. The protocol's ability to heal around link failures without stopping AI training jobs is transformative—losing one of eight links only reduces bandwidth by 12% rather than crashing the entire job. Administrators can now perform maintenance (including rebooting Tier 1 switches) without interrupting training runs. The two-tier topology reduces switch count by approximately 40% for equivalent compute engine counts while providing full-bisection bandwidth with 2/3 the optics and 3/5 the switches compared to three-tier networks. However, the dramatically increased link count (1,048,576 links for 65,536 endpoints versus 196,608 in traditional designs) may impact cable management and failure domain considerations.

**Key Technical Details**:
- MRC protocol implemented in Nvidia ConnectX-8 SmartNICs, AMD "Pollara" and "Vulcano" DPUs, Broadcom Thor Ultra SmartNICs
- Switch support on Nvidia Spectrum 4/5 (Cumulus Linux, SONiC) and Arista EOS (Broadcom Tomahawk 5 ASICs)
- Two-tier network topology: 256 XPUs per pod, 512 Tier 0 leaf switches per plane, 256 Tier 1 spine switches per plane
- Eight parallel Clos data planes with SRv6 static routing
- Tested at Oracle Stargate (Abilene, Texas) and Microsoft Azure AI (Fairwater, Wisconsin) datacenters
- Adaptive packet spraying eliminates congestion in network core, reducing flow throughput variation during synchronous training

**Tags**: [networking] [GPU] [infrastructure] [vendor] [orchestration]

**Importance**: [High]

**Why This Matters**: HPC administrators should evaluate MRC for next-generation AI cluster deployments—the protocol's fault tolerance and linear scaling characteristics could dramatically reduce operational overhead while enabling larger cluster sizes with simpler network topologies, but hardware certification and cabling infrastructure planning must account for the massive increase in network links.
