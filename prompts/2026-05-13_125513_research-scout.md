...
timestamp: 2026-05-13T12:55:13.217266
---
...
## MRC: Multipath Reliable Connection – A Scalable Ethernet Protocol for AI Clusters

**Summary**  
The article reports on the joint effort by OpenAI, Microsoft, Broadcom, AMD, and Nvidia to develop Multipath Reliable Connection (MRC), a new Ethernet‑based protocol that extends RDMA over Converged Ethernet (RoCE) with multipath, adaptive load‑balancing, and packet‑trimming capabilities. MRC leverages high‑radix switch ASICs (e.g., 51.2 Tb/s switches split into 512×100 Gb/s ports) to create eight parallel Clos data planes, enabling a two‑tier network that can support 131,072 GPUs/XPUs with only three hops between endpoints. The protocol uses IPv6 segment routing for static path selection, explicit congestion notification (ECN) for adaptive load balancing, and out‑of‑order delivery with selective retransmission. Implementation has been demonstrated on Nvidia ConnectX‑8 SmartNICs, AMD Pollara/Vulcano DPUs, Broadcom Thor Ultra SmartNICs, and on Cumulus Linux, SONiC, and Arista EOS switches.

**Source URL**  
[OpenAI, Microsoft And Friends Build A Better, More Scalable Ethernet](https://www.nextplatform.com/connect/2026/05/12/openai-microsoft-and-friends-build-a-better-more-scalable-ethernet/5239078)

**Published Date**  
2026‑05‑12T19:52:33+02:00

**Source Credibility**  
[High Credibility] – The Next Platform is a well‑known, peer‑reviewed HPC industry publication.

**Operational Impact**  
* **Network Design** – HPC admins can replace traditional three‑tier RoCE topologies with a two‑tier MRC design, reducing hop count and latency while doubling scale for the same aggregate bandwidth.  
* **Failure Resilience** – MRC’s multipath and packet‑trimming mechanisms allow continued operation when individual links fail, eliminating the need for job restarts due to link outages.  
* **Hardware Selection** – Requires high‑radix switches (e.g., 51.2 Tb/s) and SmartNICs that support MRC; admins must evaluate switch ASICs and OS support (Cumulus, SONiC, EOS).  
* **Operational Costs** – Though MRC increases the number of links (≈1.2 M vs. 0.2 M in a three‑tier design), the cost impact may be offset by reduced GPU idle time and lower power per link.  
* **Software Stack** – Integration with existing RDMA workloads is straightforward, but requires deploying MRC firmware and enabling IPv6 segment routing; monitoring tools must be updated to track multipath metrics.

**Tags**  
[networking] [GPU] [research] [vendor] [observability]

**Importance**  
[High]

**Why This Matters**  
MRC offers a practical path to scale AI clusters beyond current RoCE limits while improving fault tolerance and reducing latency, directly impacting HPC performance and operational efficiency.
